<template>
  ...
  <transition tag="div" name="approve-btn" class="inline-block absolute">
    <div v-if="this.getBtnState({remove: 'disabled'})" class="flex w-28 justify-between items-center border dark:border-gray-600 border-gray-300 space-x-1 p-2 pl-3 rounded-full group dark:bg-gray-700 bg-gray-200 select-none">
  ...
  <!-- Ready to Remove -->
  <transition tag="div" name="approve-btn" class="inline-block absolute">
    <div @click="executeRemoveLiquidity" v-if="this.getBtnState({remove: 'remove'})" class="flex w-28">
  ...
  <!-- Removing -->
  <transition tag="div" name="approve-btn" class="inline-block absolute">
    <div v-if="this.getBtnState({remove: 'removing'})" class="flex w-28">
      <div class="grab-attention-glowing"></div>
  ...
  <!-- Removed -->
  <transition tag="div" name="approve-btn" class="inline-block absolute">
    <div v-if="this.getBtnState({remove: 'removed'})" class="flex w-28 justify-between items-center border border-oswapGreen glow-oswapGreen-light-md space-x-1 p-2 pl-3 rounded-full dark:bg-oswapDark-gray bg-gray-100 cursor-default">
      <div class="flex flex-1 items-center justify-center">
