<template>
  <div class="flex pt-3">
    ...
    <div v-if="token('token1', 'token2')" class="flex flex-1 items-center justify-between">
      ...
      <div @click="goTo('swapper')" class="flex items-center space-x-2 rounded-full group hover:bg-gray-100 dark:hover:bg-oswapDark-gray bg-opacity-100 hover:bg-opacity-30 pl-3 st5 cursor-pointer">
        <p class="text-sm text-gray-500 dark:text-oswapBlue-light">next</p>
        <i class="las la-arrow-right text-lg p-2 text-gray-200 dark:text-gray-600 rounded-full bg-oswapGreen dark:bg-oswapBlue-light dark:group-hover:bg-oswapGreen group-hover:bg-oswapGreen-dark st5"></i>
      </div>
    </div>
    ...
    <div v-else class="flex flex-1 items-center justify-between cursor-default select-none">
      ...
      <div class="flex items-center space-x-2 rounded-full bg-opacity-100 pl-3">
        <p class="text-sm st5 text-gray-300 dark:text-gray-600">next</p>
